function final_fit = fit_horizon(data)
    d = data;
    ...
    RTcell = arrayfun(@(x)extendnan(x.RT,10),d,'UniformOutput',false);
    game.RT = vertcat(RTcell{:});

    % key -1(left), 1(right)
    Keycell = arrayfun(@(x)extendnan(x.key,10)*2 - 3, d,'UniformOutput',false);
    game.key = vertcat(Keycell{:});

    for ri = 1:2
        Rewardcell = arrayfun(@(x)extendnan(x.rewards(ri,:),10),d,'UniformOutput',false);
...
%     disp(horizon);
    ...
    for hi = 1:length(horizon)
        ...
        modelfree.p_lm(hi) = sum((-sign(game.dM) == game.key(:,5)) ...
            & game.cond_uncertain == 0 & game.dM ~= 0 & ind_h)...
            /sum(game.cond_uncertain == 0 & game.dM ~= 0 & ind_h);
    end
    % MLE
    % clc;
    ...
    fin = struct(                                                                   ...
        ...
        'p_high_information_h6', sub.p_hi(2),                                       ...
        'p_low_mean_h1', sub.p_lm(1),                                               ...
        'p_low_mean_h6', sub.p_lm(2),                                               ...
        'fit_info_bonus_alpha_h1', sub.fitA(1),                                     ...
        'fit_info_bonus_alpha_h6', sub.fitA(2),                                     ...
        'fit_decision_noise_sigma_13_h1', sub.fitSigma13(1),                        ...
        'fit_decision_noise_sigma_13_h6', sub.fitSigma13(2),                        ...
        ...
        'fit_spatial_bias_B_13_h6', sub.fitbias13(2),                               ...
        'fit_spatial_bias_B_22_h1', sub.fitbias22(1),                               ...
        'fit_spatial_bias_B_22_h6', sub.fitbias22(2),                               ...
        'p_high_information_h6_h1_diff', sub.p_hi(2)-sub.p_hi(1),                   ...
        'p_low_mean_h6_h1_diff', sub.p_lm(2)-sub.p_lm(1),                           ...
        'fit_info_bonus_alpha__h6_h1_diff', sub.fitA(2)-sub.fitA(1),                ...
        'fit_decision_noise_13_h6_h1_diff', sub.fitSigma13(2)-sub.fitSigma13(1),    ...
        'fit_decision_noise_22_h6_h1_diff', sub.fitSigma22(2)-sub.fitSigma22(1),    ...
        'fit_spatial_bias_B_13_h6_h1_diff', sub.fitbias13(2)-sub.fitbias13(1),      ...
